...
def modify_file(file_path, old_tag, new_tag):
    ...
    # 使用正则表达式替换第一个匹配项
    modified_content = re.sub(r'add_core_of\s*=\s*[A-Z]{3}', f'add_core_of = {new_tag}', content, count=1)
...
def main():
    ...
    for file in all_files:
        ...
        match = re.search(r'add_core_of\s*=\s*([A-Z]{3})', content)
        if match:
            ...
            print(f"在文件 {file} 中未找到 add_core_of 标签")
